...
var u=l-y.margins.top-y.margins.bottom,r=a.scaleBand().range([0,b]).padding(.1).round(1/c.length),x=q.scaleLinear().range([u,0]);l=a.axisBottom().scale(r).tickSize(2,0);k=a.axisLeft().scale(x).tickFormat(y.yFormat).ticks(5);r.domain(c.map(function(a){return a[e]}));x.domain([0,y.yMax||a.max(c,function(a){return a[h]})]);var t=g.append("g").attr("class","axis");t.call(l);var w=Math.round(t.node().getBBox().height),z=Math.round(t.node().getBBox().width);u-=w;b-=Math.max(0,z-b);t.remove();t=g.append("g").attr("class",
"axis");t.call(k);w=Math.round(t.node().getBBox().width);b-=w;t.remove();r.range([0,b]);x.range([u,0]);g=g.append("g").attr("transform","translate(\n\t          "+(y.margins.left+w)+",\n\t          "+y.margins.top+"\n\t        )");g.append("g").attr("class","x axis").attr("transform","translate(0, "+u+")").call(l).selectAll(".tick text").style("text-anchor",y.textAnchor).attr("transform",function(a){return"rotate("+y.rotate+")"});y.wrap&&g.selectAll(".tick text").call(this.wrap,r.bandwidth());g.append("g").attr("class",
"y axis").attr("transform","translate(0, 0)").call(k);g.selectAll(".bar").data(c).enter().append("rect").attr("class","bar").attr("x",function(a){return r(a[e])}).attr("width",r.bandwidth()).attr("y",function(a){return x(a[h])}).attr("height",function(a){return u-x(a[h])}).attr("title",function(a){var p=a[e]+": "+d.formatters.commaseparated(a[h],",");return p=0<m?p+(" ("+d.formatters.formatpercent(a[h]/m)+")"):p+(" ("+d.formatters.formatpercent(0)+")")}).style("fill",function(a){return y.colors(a[e])}).on("mouseover",
function(a){return f.show(a,event.target)}).on("mouseout",f.hide).exit().remove();y.showLabels&&g.selectAll(".barlabel").data(c).enter().append("text").attr("class","barlabel").text(function(a){return d.formatters.formatpercent(a[h]/m)}).attr("x",function(a){return r(a[e])+r.bandwidth()/2}).attr("y",function(a){return x(a[h])-3}).attr("text-anchor","middle")};$jscomp.global.Object.defineProperties(c.prototype,{formatters:{configurable:!0,enumerable:!0,get:function(){return{commaseparated:a.format(","),
formatpercent:a.format(".1%")}}}});return c});
define("boxplot",["d3","d3-tip","d3-scale","chart"],function(a,n,q,h){var c=function(a){return h.apply(this,arguments)||this};$jscomp.inherits(c,h);c.prototype.render=function(c,g,b,l,k){k=this.getOptions(k);g=this.createSvg(g,b,l);var d=this.formatters.formatSI(3),y=n().attr("class","d3-tip").offset([-10,0]).html(function(a){return"\x3ctable class\x3d'boxplotValues'\x3e\n\t          \x3ctr\x3e\n\t            \x3ctd\x3eMax:\x3c/td\x3e\n\t            \x3ctd\x3e"+d(a.max)+"\x3c/td\x3e\n\t          \x3c/tr\x3e\n\t          \x3ctr\x3e\n\t            \x3ctd\x3eP90:\x3c/td\x3e\n\t            \x3ctd\x3e"+
...
define("donut","d3 d3-tip d3-shape d3-drag numeral chart".split(" "),function(a,n,q,h,c,e){var g=function(a){return e.apply(this,arguments)||this};$jscomp.inherits(g,e);g.prototype.render=function(b,g,k,d,e){function f(){var b=a.select(this);b.attr("initialX",event.x);b.attr("initialY",event.y)}function l(){var b=a.select(this),p=event.y-parseFloat(b.attr("initialY"));if(isNaN(p))return!1;b.attr("transform","translate(\n\t        "+parseFloat(b.attr("initialPositionX"))+",\n\t        "+(parseFloat(b.attr("initialPositionY"))+
p)+"\n\t      )")}function y(){var b=a.select(this);b.transition().duration(300).attr("transform","translate(\n\t          "+b.attr("initialPositionX")+",\n\t          "+b.attr("initialPositionY")+"\n\t        )")}var m=this,u=this.getOptions(e);e=this.createSvg(g,k,d);var r=0;b.forEach(function(a){r+=+a.value});g=this.donutDefaultTooltip(function(a){return a.label},function(a){return c(a.value).format("0,0")},function(a){return m.formatters.formatpercent(0!=r?a.value/r:0)});var x=n().attr("class",
"d3-tip").direction("s").offset([3,0]).html(g);e.call(x);if(0<b.length){g=e.append("g").attr("id","chart");var t=h.drag().on("drag",l).on("start",f).on("end",y);e=e.append("g").attr("class","legend").call(t);e.selectAll("rect").data(b).enter().append("rect").attr("x",0).attr("y",function(a,b){return 15*b}).attr("width",10).attr("height",10).style("fill",function(a){return u.colors(a.id)});var w=0;t=e.selectAll("g.legend-item").data(b).enter().append("g").attr("class","legend-item");t.append("text").attr("x",
12).attr("y",function(a,b){return 15*b+9}).text(function(a){return a.label});t.append("title").attr("x",12).attr("y",function(a,b){return 15*b+9}).text(function(a){return a.label});t.each(function(){var a=this.getBBox().width;a>w&&w+a<.75*k&&(w=a)});w+=12;e.attr("transform","translate(\n\t            "+(k-w-u.margins.right)+",\n\t            "+u.margins.top+"\n\t          )").attr("initialPositionX",k-w-u.margins.right).attr("initialPositionY",u.margins.top);g.attr("transform","translate("+(k-w)/
2+", "+d/2+")");e=Math.min(d,k-w)/2-u.margins.top;d=Math.min(d,k-w)/6-u.margins.top;d=q.arc().innerRadius(d).outerRadius(e);e=a.pie().value(function(a){return 0<a.value?Math.max(a.value,.015*r):0});g.selectAll("g.slice").data(e(b)).enter().append("g").attr("class","slice").append("path").attr("fill",function(a){return u.colors(a.data.id)}).attr("stroke","#fff").attr("stroke-width",5).attr("title",function(a){return a.label}).on("mouseover",function(a){return x.show(a.data,event.target)}).on("mouseout",
x.hide).attr("d",d)}else e.append("text").attr("transform","translate("+k/2+", "+d/2+")").style("text-anchor","middle").text("No Data")};$jscomp.global.Object.defineProperties(g.prototype,{formatters:{configurable:!0,enumerable:!0,get:function(){return{formatpercent:a.format(".1%")}}}});return g});
define("histogram",["d3","d3-tip","d3-scale","numeral","chart"],function(a,n,q,h,c){var e=function(a){return c.apply(this,arguments)||this};$jscomp.inherits(e,c);e.mapHistogram=function(a){for(var b=[],g=a.MIN,k=a.INTERVAL_SIZE,d=this.normalizeDataframe(a.DATA),c=0;c<=a.INTERVALS;c+=1){var f={};f.x=g+1*c*k;f.dx=k;f.y=d.COUNT_VALUE[d.INTERVAL_INDEX.indexOf(c)]||0;b.push(f)}return b};e.prototype.drawBoxplot=function(a,b,c,k){c=this.xScale;var d=k/2;b.LIF!==b.q1&&(a.append("line").attr("class","bar").attr("x1",
c(b.LIF)).attr("y1",k/2-d/2).attr("x2",c(b.LIF)).attr("y2",k/2+d/2),a.append("line").attr("class","whisker").attr("x1",c(b.LIF)).attr("y1",k/2).attr("x2",c(b.q1)).attr("y2",k/2));a.append("rect").attr("class","box").attr("x",c(b.q1)).attr("width",c(b.q3)-c(b.q1)).attr("height",k);a.append("line").attr("class","median").attr("x1",c(b.median)).attr("y1",0).attr("x2",c(b.median)).attr("y2",k);b.UIF!==b.q3&&(a.append("line").attr("class","bar").attr("x1",c(b.UIF)).attr("y1",k/2-d/2).attr("x2",c(b.UIF)).attr("y2",
k/2+d/2),a.append("line").attr("class","whisker").attr("x1",c(b.q3)).attr("y1",k/2).attr("x2",c(b.UIF)).attr("y2",k/2))};e.prototype.render=function(c,b,e,k,d){var g={ticks:10,yScale:q.scaleLinear(),boxplotHeight:10};d=this.getOptions(g,d);b=this.createSvg(b,e,k);this.xScale={};c=c||[];var f=n().attr("class","d3-tip").offset([-10,0]).html(function(a){return h(a.y).format("0,0")});b.call(f);var l=0;g=0;d.xLabel&&(l=b.append("g").attr("transform","translate("+e/2+", "+(k-d.margins.bottom)+")"),l.append("text").attr("class",
"axislabel").style("text-anchor","middle").text(d.xLabel),l=l.node().getBBox().height);d.yLabel&&(g=b.append("g").attr("transform","translate(\n\t            "+d.margins.left+",\n\t            "+((k-d.margins.bottom-d.margins.top)/2+d.margins.top)+"\n\t          )"),g.append("text").attr("class","axislabel").attr("transform","rotate(-90)").attr("y",0).attr("x",0).attr("dy","1em").style("text-anchor","middle").text(d.yLabel),g=1.5*g.node().getBBox().width);e=e-d.margins.left-d.margins.right-g;var v=
k-d.margins.top-d.margins.bottom-l,m=this.xScale=q.scaleLinear().domain(d.xDomain||[a.min(c,function(a){return a.x}),a.max(c,function(a){return a.x+a.dx})]).range([0,e]);k=a.axisBottom().scale(m).ticks(d.ticks).tickFormat(d.xFormat);var u=d.yScale.domain([0,d.yMax||a.max(c,function(a){return a.y})]).range([v,0]);l=a.axisLeft().scale(u).ticks(4).tickFormat(d.yFormat);var r=b.append("g").attr("class","axis");r.call(k);var x=Math.round(r.node().getBBox().height),t=Math.round(r.node().getBBox().width);
...
define("scatterplot",["d3","d3-tip","chart"],function(a,n,q){var h=function(a){return q.apply(this,arguments)||this};$jscomp.inherits(h,q);h.prototype.render=function(c,e,g,b,h){var k={xFormat:this.formatters.formatSI(3),yFormat:this.formatters.formatSI(3),interpolate:a.curveLinear,seriesName:"SERIES_NAME",xValue:"xValue",yValue:"yValue",cssClass:"lineplot",ticks:10,showSeriesLabel:!1,labelIndexDate:!1,colorBasedOnIndex:!1,showXAxis:!0,tooltip:function(a){return"\x3cdiv\x3eSeries: "+a.seriesName+
"\x3c/div\x3e\n\t\t\t\t\t\x3cdiv\x3eX: "+a[d.xValue]+"\x3c/div\x3e\n\t\t\t\t\t\x3cdiv\x3eY: "+a[d.yValue]+"\x3c/div\x3e\n\t\t\t\t\t"}},d=this.getOptions(k,h);d.colors=h.colors?a.scaleOrdinal(Object.values(h.colors)).domain(Object.keys(h.colors)):a.scaleOrdinal(a.schemeCategory20).domain(c.map(function(a){return a.name}));e=this.createSvg(e,g,b);if(0<c.length){c[0].hasOwnProperty("values")||(c=[{name:"",values:c}]);var l=n().attr("class","d3-tip").offset([-10,0]).html(d.tooltip);e.call(l);h=k=0;if(d.xLabel){var f=
e.append("g").attr("transform","translate("+g/2+","+(b-d.margins.bottom)+")");f.append("text").attr("class","axislabel").style("text-anchor","middle").text(d.xLabel);f=f.node().getBBox();k+=f.height}d.yLabel&&(h=e.append("g").attr("transform","translate("+d.margins.left+","+((b-d.margins.bottom-d.margins.top)/2+d.margins.top)+")"),h.append("text").attr("class","axislabel").attr("transform","rotate(-90)").attr("y",0).attr("x",0).attr("dy","1em").style("text-anchor","middle").text(d.yLabel),f=h.node().getBBox(),
h=1.5*f.width);f=0;if(d.showLegend){var q=e.append("g").attr("class","legend"),v=0;c.forEach(function(a,b){q.append("rect").attr("x",0).attr("y",15*b).attr("width",10).attr("height",10).style("fill",d.colors(a.name));a=q.append("text").attr("x",12).attr("y",15*b+9).text(a.name);v=Math.max(a.node().getBBox().width+12,v)});q.attr("transform","translate("+(g-d.margins.right-v)+",0)");f+=v+5}g=g-d.margins.left-d.margins.right-h-f;b=b-d.margins.top-d.margins.bottom-k;var m=d.xScale||a.scaleLinear().domain([a.min(c,
function(b){return a.min(b.values,function(a){return a[d.xValue]})}),a.max(c,function(b){return a.max(b.values,function(a){return a[d.xValue]})})]);k=a.axisBottom().scale(m).ticks(d.ticks);d.tickFormat?k.tickFormat(d.tickFormat):k.tickFormat(d.xFormat);"function"===typeof m.rangePoints?m.rangePoints([0,g]):m.range([0,g]);var u=d.yScale||a.scaleLinear().domain([0,a.max(c,function(b){return a.max(b.values,function(a){return a[d.yValue]})})]).range([b,0]);f=a.axisLeft().scale(u).tickFormat(d.yFormat).ticks(4);
